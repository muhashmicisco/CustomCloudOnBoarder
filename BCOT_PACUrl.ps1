...
foreach ($device in $devices) {
    ...
    try {
        ...
        if ($useProxy) {
            $proxyXml = "<Configuration><NetworkServices><HTTP><Mode>HTTP+HTTPS</Mode><Proxy><PACUrl>$proxyUrl</PACUrl><Mode>PACUrl</Mode></Proxy></HTTP></NetworkServices></Configuration>"
